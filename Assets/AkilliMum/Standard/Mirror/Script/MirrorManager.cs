...
namespace AkilliMum.Standard.Mirror
{
    ...
    public class MirrorManager : MonoBehaviour
    {
        ...
        public virtual IList<Camera> RenderReflective(Camera sentCamera = null, bool invert = true, bool renderCam = true)
        {
            ...
            {  //draw the scene twice for VR
                ...
                //_stereoEye = 0;
                var camera0 = RenderMe(StereoTargetEyeMask.Left, sentCamera, invert, renderCam);
                cameras.Add(camera0);

                //_stereoEye = 1;
                var camera1 = RenderMe(StereoTargetEyeMask.Right, sentCamera, invert, renderCam);
                cameras.Add(camera1);
            }
            else
            {
                ...
                if (ClipEye == ClipEye.Left)
                    camera0 = RenderMe(StereoTargetEyeMask.Left, sentCamera, invert, renderCam);
                else if (ClipEye == ClipEye.Right)
                    camera0 = RenderMe(StereoTargetEyeMask.Right, sentCamera, invert, renderCam);
                else
                    camera0 = RenderMe(StereoTargetEyeMask.None, sentCamera, invert, renderCam);
                cameras.Add(camera0);
            }
        ...
        private Camera RenderMe(StereoTargetEyeMask stereoTargetEyeMask, Camera sentCamera, bool invert = true, bool renderCam = true)
        {
    ...
    {
        StandAlone = 0,
        VR = 10, //do not change used in shader
        //OpenXR = 20,
        //Oculus = 30,
    ...
    public class RendererManager : ManagerBase
    {
                ...
                foreach (var ren in _renderers)
                {
                    if (ren != null)
                    {
                        foreach (var mat in ren.sharedMaterials)
                        {
                            ...
                            if (mat != null &&
                                mat.shader.name == "AkilliMum/Standard/Mirror/Complex")
        ...
        public void UpdateMaterials(RenderTextureManager renderTextureManager, CameraManager cameraManager)
        {
            ...
            foreach (var mat in _materials)
            {
                ...
                if (mat.HasProperty("_Platform"))
                    mat.SetFloat("_Platform", (int)cameraManager.GetSettings().Platform);
                ...
                if (cameraManager.GetSettings().EnableDepthBlur)
                {
                    ...
                    if (cameraManager.GetSettings().DepthBlurIterations % 2 == 1) //again a hack:)
                    {
                        if (mat.HasProperty("_ReflectionTex"))
                            mat.SetTexture("_ReflectionTex", renderTextureManager.LeftOrCenterRT3);

                        if (mat.HasProperty("_ReflectionTexOther"))
                            mat.SetTexture("_ReflectionTexOther", renderTextureManager.RightRT3);
                    }
                    else
                    {
                        if (mat.HasProperty("_ReflectionTex"))
                            mat.SetTexture("_ReflectionTex", renderTextureManager.LeftOrCenterRT2);

                        if (mat.HasProperty("_ReflectionTexOther"))
                            mat.SetTexture("_ReflectionTexOther", renderTextureManager.RightRT2);
                    }
                }
                else
                {
                    if (mat.HasProperty("_ReflectionTex"))
                        mat.SetTexture("_ReflectionTex", renderTextureManager.LeftOrCenterRT1);

                    if (mat.HasProperty("_ReflectionTexOther"))
                        mat.SetTexture("_ReflectionTexOther", renderTextureManager.RightRT1);
                }

                if (mat.HasProperty("_ReflectionTexDepth"))
                    mat.SetTexture("_ReflectionTexDepth", renderTextureManager.LeftOrCenterDepthRT1);

                if (mat.HasProperty("_ReflectionTexOtherDepth"))
                    mat.SetTexture("_ReflectionTexOtherDepth", renderTextureManager.RightDepthRT1);
            }
        }
...
#pragma warning restore 618
                ...
                ////so our fix should be half of this separation!
                var fix = distance; // no need to divide by 2 on standard???? what the?
                //on standard rendering, is that using always stereo seperation of camera?
                //float fix = 0.064F; //_mainCamera.stereoSeparation != 0 ? _mainCamera.stereoSeparation + 0.01F : .032F; //for normal 64 mm IPD

                //left eye fix is zero for standard? what the :)
                worldToCameraMatrix[12] += stereoTargetEyeMask == StereoTargetEyeMask.Left ? 0 : -fix;
            }
    ...
    public class OptionManager : ManagerBase
    {
        ...
        public void Start()
        {
            ...
            {
                //save previous
                _previousMaxAdditionalLightsCount = QualitySettings.pixelLightCount;
                //disable
        ...
        public void End()
        {
            ...
            {
                //revert
                QualitySettings.pixelLightCount = _previousMaxAdditionalLightsCount;
            }
